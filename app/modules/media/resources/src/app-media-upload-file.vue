<template>
<div class="app-media-upload uk-flex uk-flex-stretch">
    <div class="app-media-upload-dropzone uk-width-1-1 uk-flex uk-flex-wrap uk-flex-middle uk-padding" ref="dropzone" @drop.prevent="fileDrop" @dragover.prevent>
        ...
        <div class="app-media-upload-button uk-width-1-1 uk-margin-top uk-text-center">
            <button :class="{ 'uk-button uk-button-danger': true, 'uk-disabled': files.length == 0 }" @click="fileRemove">{{ $t('liro-media.upload.remove_all') }}</button>
            <button :class="{ 'uk-button uk-button-success': true, 'uk-disabled': files.length == 0 }" @click="fileUpload">{{ $t('liro-media.upload.start') }}</button>
        </div>
